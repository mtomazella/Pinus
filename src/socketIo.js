    ...
    /* WebSocket Routes */
        ...
        io.on( 'connect', ( socket ) => {
            ...
            socket.on( 'disconnect', ( ) => {
                if ( socket.user && this.clientSockets[ socket.user.type ][ socket.user.id ] == socket ) delete this.clientSockets[ socket.user.type ][ socket.user.id ];
                for ( let i = this.supportQueue.length-1; i >= 0; i-- ) if ( socket.user && this.supportQueue[i].userId == socket.user.id ) this.supportQueue.splice( i, 1 )
